 ...
 * RMS history.
 */
export function linearLayout(yOffset: number, height: number, z: number): LineLayoutFn {
  return (i, n, value) => {
    const x = n <= 1 ? 0 : (i / (n - 1)) * 2 - 1;
    return new Vector3(x, yOffset + value * height, z);
  };
}
 ...
 * bin (n-1) maps to x = +1.
 */
export function logSpectrumLayout(yOffset: number, height: number, z: number): LineLayoutFn {
  return (i, n, value) => {

...
export class ParticleView implements Component {
  ...
  update(): void {
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      // setLinvel below REPLACES velocity outright instead of blending with the
      // previous one, so particles carry no momentum across frames — the fields
      // alone determine motion each step. orbitalInit writes initial velocity
      // using the same formula so spawn is consistent with the field.
